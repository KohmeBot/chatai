...
网页读取会拒绝本机、内网和链路本地地址，并限制超时、响应大小和重定向次数。
...
消息数不足时不会丢弃，会继续积累到后续周期。印象和 token 用量保存在插件数据库中；短期群聊上下文保存在内存中，每个群最多保留 500 条。
...
```text
[Agent][group=123 user=456 step=1/8] 开始模型决策
[Agent][group=123 step=1][思维链] ...
[Agent][group=123 step=1][模型输出] ...
[Agent][group=123 step=1][工具调用] read_group_context args={"limit":20}
[Agent][group=123 step=1][工具结果] read_group_context result=...
[Agent][group=123 step=2][最终输出] ...
```

只有模型接口实际返回 `reasoning_content` 时才会打印思考内容。供应商或模型不提供该字段时，日志会显示“模型未返回 reasoning_content”。工具结果过长时会在 6000 个字符处截断，避免单条日志无限增长。
