...
当前事件、历史上下文和追问回复包含 `raw_message`（原始 CQ 文本）、`raw_segments`（原始 JSON 消息段数组）和 `quoted_raw_segments`（引用消息段数组）。接收端保留框架提供的原始数组，包括未知消息段和嵌套内容。启动时通过现有数据库自动迁移增加字段；旧记录没有原始消息数据，无法补回。
...
工具使用 `Message []Segment`，`data` 参数值为字符串，普通消息段直接发送到当前群，不自动加引用或改写内容。合并转发先使用 `read_forward_message` 读取 `forward.data.id`，再构造全部为 `node` 的数组；此时自动使用 `send_group_forward_msg`。自定义节点的 `content` 是消息数组的 JSON 字符串，发送前展开为真正嵌套数组：
