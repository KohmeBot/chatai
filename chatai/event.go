...
func (c *ChatPlugin) SetOnPoke(engine plugin.Engine) {
	...
	engine.OnNotice(c.env.Groups().Rule()).Handle(func(ctx *zero.Ctx) {
		...
		gopool.Go(func() {
			...
			if c.conf.Favor {
				err = favor.ProcessFavorResponse(db, ctx.Event.UserID, res)
				if err != nil {
					return
				}
			}
...
func (c *ChatPlugin) onResponse(ctx *zero.Ctx, request *model.Request, response *model.Response, err error) {
	...
	if c.conf.Favor {
		...
		err = favor.ProcessFavorResponse(db, ctx.Event.UserID, response)
		if err != nil {
			return
		}
