...
const favorFormat = "你的回复要以json的格式给我,字段有两个,第一个是answer(string)这是你回答的内容,第二个是favor(int)这是你根据提问给用户增加或者减少的好感度,每次最多增加10点,减少30点,接下来我会让你扮演角色和群友对话,我会告诉你当前你对他的好感度(%d-%d),根据好感度的多少来回答问题\n%s"
...
func (f Favor) WithSystem(system string) string {
	...
	s := fmt.Sprintf(favorFormat, FavorMin, FavorMax, system)
	return s
}
